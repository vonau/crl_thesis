...
sample_file_path = f'../Data/Samples/data_cube-drag_{nTimeSteps}tsteps_' + now.strftime("%H%M") + '_robustness/'
...
targets[:,0] = np.random.rand(tot_simulations_needed)*0.5+0.5
targets[:,1] = 0.0268342693608721
targets[:,2] = np.random.rand(tot_simulations_needed)*0.5+0.5
...
print("Broken indices:")
print(broken_indices)

###############################################
#SAMPLE NEW TARGETS
print("SAMPLING NEW TARGETS...")
input[:,0] = np.random.rand(samplenum)*0.5+0.5
input[:,1] = 0.0268342693608721
input[:,2] = np.random.rand(samplenum)*0.5+0.5
p_init = p_init_0
