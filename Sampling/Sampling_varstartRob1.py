...
sample_file_path = f'../Data/Samples/data_varstart_{nTimeSteps}tsteps' + now.strftime("%H%M") + '_robustness1/'
...
targets[:,0] = np.random.rand(tot_simulations_needed)+1
targets[:,1] = np.random.rand(tot_simulations_needed)+1
targets[:,2] = np.random.rand(tot_simulations_needed)+1
...
input[:,0] = np.random.rand(samplenum)+1
input[:,1] = np.random.rand(samplenum)+1
input[:,2] = np.random.rand(samplenum)+1
p_init = p_init_0
c= 0
