...
sample_file_path = f'../Data/Samples/data_varstart_{nTimeSteps}tsteps' + now.strftime("%H%M") + '_robustness/'
...
print(f'{tot_simulations_needed} simulations needed')

input = np.zeros((samplenum,input_size))
targets = np.zeros((tot_simulations_needed,3))
targets[:,0] = np.random.rand(tot_simulations_needed)*0.5+1
targets[:,1] = np.random.rand(tot_simulations_needed)*0.5+1
targets[:,2] = np.random.rand(tot_simulations_needed)*0.5+1
...
for i_ in range(tot_simulations_needed):
    ...
    input[i_*samples_per_sim,3:6] = dyn.q0
    input[i_*samples_per_sim,6:9] = dyn.qdot0
    input[i_*samples_per_sim,9:12] = dyn.qddot0
    input[i_*samples_per_sim,12:15] = dyn.p0
...
input[:,0] = np.random.rand(samplenum)*0.5+1
input[:,1] = np.random.rand(samplenum)*0.5+1
input[:,2] = np.random.rand(samplenum)*0.5+1
p_init = p_init_0
c= 0
...
for b in range(filenum):
    ...
    for i_2 in range(batch_size):
        ...
        dyn.loadJson(dyn_json, nTimeSteps)
        dyn.p0 = input[b*batch_size+i_2,12:15]
