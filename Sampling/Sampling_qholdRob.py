...
use_case = 'qhold'
sample_file_path = f'../Data/Samples/data_qhold_{nTimeSteps}tsteps_' + now.strftime("%H%M") + '_robustness/'
...
# sample p_now
input[:,6] = np.random.rand(samplenum)*4-1
input[:,7] = np.random.rand(samplenum)*4-1
input[:,8] = np.random.rand(samplenum)*4-1
