...
if samplenum_target == 15000:
    ...
    model_file_path_passive = '../Trained_Models/state_dict/Model_statedict_passive_' + use_case + f'_{nTimeSteps}tsteps_latest_{samplenum_target}.pt'

sample_file_path = path + f'Data/Samples/data_cube-drag_{nTimeSteps}tsteps_0830active/'
...
number_of_files = len(os.listdir(sample_file_path))-2
...
pnow = np.zeros((samplenum, dyn.nParameters))
for filenum in range(number_of_files):
    with open(sample_file_path + f'data_{filenum}.json') as json_file:
        ...
        for i, q_i in enumerate(data['q']):
            ...
            pmq = np.array([q_i[0],q_i[2], q_i[0], q_i[2]])
        ...
        for i, p_now_i in enumerate(data['p_now']):
            ...
            pnow[filenum*filesize+i, :] = np.array(p_now_i)
...
data[:, 15:18] = minmaxscale(data[:, 15:18], 9.945169827818612)
data[:, 18:21] = minmaxscale(data[:, 18:21], 833.9826944026114)
...
class Simulate(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        for i_b in range(bs):
            ...
            dyn.p0 = data_input_2[i_b, 21:25] + [data_input_2[i_b, 3],data_input_2[i_b, 5], data_input_2[i_b, 3], data_input_2[i_b, 5]]
            state = dyn.q(p_2[i_b, :])
...
for e in range(epochs):
    for b in range(batch):
        ...
        pnow_tensor = torch.tensor(pnow[b*minibatch_size:b*minibatch_size+minibatch_size,:]).float()
        ...
        p_start_error = weight_c2*criterion(p_b[:, 0:dyn.nParameters], pnow_tensor)
        ...
        optimizer.step()
    if e%(epochs/10) == 0:
        print(f'epoch: {e:3}/{epochs}  loss: {loss.item():10.8f}   q_end_loss: {q_end_error.item():10.8f}')
    if loss < passiveLoss and e >= 1:
        break
...
epoch_i = np.arange(len(losses)*batch)/batch
