...
sample_file_path = path + f'Data/Samples/data_cube-drag_{nTimeSteps}tsteps_1814/'
simulation_file_path = path + 'Data/Simulations/cube-drag.sim'
objective_file_path = path + f'Data/Objectives/cube-drag.obj'
...
# sample_file_path = f'../Data/Samples/data_cube-drag_{nTimeSteps}tsteps_1814/'
...
print(f'Shape of DATA_TEST after removing failed samples:{data.shape}')
print(f'Shape of P_TEST after removing failed samples:{p_data.shape}')
...
#LOAD TEST SAMPLES
# 0:3 target, 3:9 q, 9:15 qdot, 15:19 p_now
number_of_files_test = len(os.listdir(sample_file_path + 'data_test/'))-2
...
for filenum in range(number_of_files_test):
    with open(sample_file_path + f'data_test/data_{filenum}.json') as json_file:
        ...
        for i, p_now_i in enumerate(data['p_now']):
            input_test[filenum*filesize+i, 21:25] = np.array(p_now_i) - [input_test[filenum*filesize+i, 3],input_test[filenum*filesize+i, 5], input_test[filenum*filesize+i, 3], input_test[filenum*filesize+i, 5]]
...
class Simulate(torch.autograd.Function):
    ...
    def forward(ctx, input, data_input):
        ...
        for i_f in range(bs):
            ...
            dyn.p0 = data_input[i_f, 21:25] + [data_input[i_f, 3],data_input[i_f, 5], data_input[i_f, 3], data_input[i_f, 5]]
            state = dyn.q(p_[i_f,:])
    ...
    def backward(ctx, grad_output):
        ...
        for i_b in range(bs_):
            ...
            dyn.p0 = data_input[i_b, 21:25] + [data_input[i_b, 3],data_input[i_b, 5], data_input[i_b, 3], data_input[i_b, 5]]
            state = dyn.q(p_2[i_b, :])
...
class PassiveLearn(nn.Module):
    ...
    def __init__(self, n_in, out_sz):
        ...
        # self.H3 = nn.Linear(hiddenlayers[2], out_sz)
...
if samplenum_target == 15000:
    ...
    #Save entire Model
    torch.save(model, model_file_path + 'Model_passive_' + use_case + f'_{nTimeSteps}tsteps_{samplenum}s_{epochs}e_{minibatch_size}bs_{LRdecay}lr_' + timestr + '.pt')
    torch.save(model, model_file_path + 'Model_passive_' + use_case + f'_{nTimeSteps}tsteps_latest.pt')

    #Save parameters of Model
    torch.save(model.state_dict(), model_file_path + 'state_dict/Trained_Model_statedict_passive_' + use_case + f'_{nTimeSteps}tsteps_{samplenum}s_{epochs}e_{minibatch_size}bs_{LRdecay}lr_' + timestr + '.pt')
    ...
    traced_script_module.save(model_file_path + 'Serialized_Models/Serialized_model_passive_' + use_case + f'_{nTimeSteps}tsteps_{samplenum}s_{epochs}e_{minibatch_size}bs_{LRdecay}lr_' + timestr + '.pt')
    print('Model saved')

else:
    timestr = time.strftime("%m%d")
    #Save entire Model
    torch.save(model, model_file_path + 'Model_passive_' + use_case + f'_{nTimeSteps}tsteps_{samplenum}s_{epochs}e_{minibatch_size}bs_{LRdecay}lr_' + timestr + f'_{samplenum_target}.pt')
    torch.save(model, model_file_path + 'Model_passive_' + use_case + f'_{nTimeSteps}tsteps_latest_{samplenum_target}.pt')

    #Save parameters of Model
    torch.save(model.state_dict(), model_file_path + 'state_dict/Model_statedict_passive_' + use_case + f'_{nTimeSteps}tsteps_{samplenum}s_{epochs}e_{minibatch_size}bs_{LRdecay}lr_' + timestr + f'_{samplenum_target}.pt')
    ...
    traced_script_module.save(model_file_path + 'Serialized_Models/Serialized_model_passive_' + use_case + f'_{nTimeSteps}tsteps_{samplenum}s_{epochs}e_{minibatch_size}bs_{LRdecay}lr_' + timestr + f'_{samplenum_target}.pt')
    print('Model saved')
...
with torch.no_grad():
    for i in range(samplenum_test):
        ...
        pmq = torch.tensor([input[i, 3], input[i, 5], input[i, 3], input[i, 5]])
