...
learning_rate = 0.001
epochs_s = [200, 300]
minibatch_size = [20, 50]
LRdecay_s = [0.7]
hiddenlayers_s = [[280, 380], [280, 350]] 
            ...
            for h in hiddenlayers_s:
                ...
                with torch.no_grad():
                    for i in range(samplenum_target_test):
                        ...
                        p_start_error = weight_c2*criterion2(p_val[0:dyn.nParameters], input_testTensor[i,21:25] + pmq)
                        q_error = weight_c1*criterion2(q_val[0, dyn.nDofs*(nTimeSteps-1):dyn.nDofs*(nTimeSteps-1)+3], input_testTensor[i, 0:3])
                        smoothness_error = weight_c3*criterion2(p_val[0:dyn.nParameters*(nTimeSteps-1)], p_val[dyn.nParameters:dyn.nParameters*nTimeSteps])
