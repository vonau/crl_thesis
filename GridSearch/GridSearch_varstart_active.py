            ...
            for h in hiddenlayers_s:
                ...
                with torch.no_grad():
                    for i in range(samplenum_target_test):
                        ...
                        p_start_error = weight_c2*criterion2(p_val[0:dyn.nParameters], input_testTensor[i,12:15] + input_testTensor[i,3:6])
                        q_error = weight_c1*criterion2(q_val[0, -3:], input_testTensor[i, 0:3])
...
with open(f'Gridsearch_scores_vastart_active_{layerz}.json', 'w') as outfile:
